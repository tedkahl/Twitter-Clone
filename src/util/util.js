...
export function like(id,operation)
{
    return new Promise((resolve, reject) => {
        axios.get(`${server}/api/tweets/${operation}/${id}`, { withCredentials: true })
...
{
    return new Promise((resolve, reject) => {
        axios.get(`${server}/api/tweets/retweet/${id}`,{withCredentials:true})
            .then(res => resolve(res))
            .catch(err => reject(err))
    });
}
...
{
    return new Promise((resolve, reject) => {
        axios.get(`${server}/api/users/${operation}/${id}`,{withCredentials:true})
            .then(res => resolve(res))
            .catch(err => reject(err))
    });
}
