...
export default class UserPage extends Component{
    ...
    getUpage(){
        ...
        .then(found=>{
            found.data.other_users.push({_id:found.data.user._id,username:found.data.user.username});
            let names=getNames(found.data.other_users);
            let tl=handleTimeline(this.state.me.id,found.data.user.tweets,
    ...
    loadTimeline(){ if(this.state.loaded) 
                        return(
    ...
    followButton()
    {
        ...
        if(this.state.user.followers.includes(this.state.me.id)){
            return(<button className="btn btn-info"
                            onClick={()=>{
                               follow(this.state.user.id, "unfollow")
                                .then(()=>{
                                    let f=findAndRemove(this.state.me.following,this.state.user.id);
            ...
            return (<button className="btn btn-primary"
                onClick={() => {
                    follow(this.state.user.id, "follow")
                        .then(() => {
            ...
            <div className="container">
                <div>
                    <div className="d-flex">
                        <h2 className="d-flex justify-content-start">{this.state.user.username}</h2>
                        <div className="d-flex justify-content-end">{this.followButton()}</div>
                    </div>
