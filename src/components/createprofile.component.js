...
export default class CreateProfile extends Component {
    ...
    componentDidMount(){
        axios.get(`${server}/api/users/popular/5`)
            .then(response=>{
                if(response.data.length>0){
                    this.setState({
                        follow_suggestions: response.data.map(user=>{return {value:user._id,label:user.username}})
                    });
    ...
    onChangeFollowing = selectedOption=>{
        let newfollowing=selectedOption?selectedOption.map(option=>option.value):null;
        this.setState({
    ...
    onSubmit(e){
        ...
        console.log(user);
        axios.post(`${server}/api/users/`,user,{withCredentials:true}) 
