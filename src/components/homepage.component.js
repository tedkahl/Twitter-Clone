...
export default class HomePage extends Component {
  ...
  componentDidMount() {
    axios
      .get(`${server}/api/users/homepage/${this.state.limit}`, {
      ...
      .catch((err) => {
        if (err.response.status == 401) window.location = "/signin";
      });
  }
  ...
  postTweet(text) {
    ...
    axios
      .post(`${server}/api/tweets/`, data, { withCredentials: true })
      .then((res) => {
  ...
  loadTweetForm() {
    return this.state.loaded ? <TweetForm /> : <p>Loading...</p>;
  }
    ...
    return (
      <div className="container">
        ...
        <div className="container">
          ...
          <div className="timeline">{this.loadTimeline()}</div>
        </div>
      </div>
    );
  }
