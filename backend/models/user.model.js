...
userSchema.methods.generateAuthToken = function(){
    const token= jwt.sign({_id:this._id, username:this.username,isAdmin:this.isAdmin}, process.env.myprivatekey);
    return token;
}
...
function validateUser(user) {
    let count=0;
    if(user.followers)
        for(x of user.followers)count++;
    const schema = Joi.object({
      id: Joi.string(),
      username: Joi.string().pattern(/^((?!\s).)*$/).min(3).max(50).required(),
