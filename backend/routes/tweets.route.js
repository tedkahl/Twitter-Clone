...
router.post("/update/:id", auth, (req, res) => {
  let tweet = Tweet.findById(req.params.id).catch((err) =>
...
router.get("/unlike/:id", auth, async (req, res) => {
  let r = await Tweet.updateOne(
    { _id: req.params.id, liked_by: { $in: req.user._id } },
    {
      $inc: { likes: -1 },
...
router.get("/like/:id", auth, async (req, res) => {
  ...
  let r;
  try {
    r = await Tweet.updateOne(
      { _id: req.params.id, liked_by: { $nin: req.user._id } },
      {
        $inc: { likes: 1 },
...
router.post("/unretweet/:id", auth, async (req, res) => {
  let r = await Promise.all([
    Tweet.find({ _id: req.params.id })
      .where("tweeted_by")
      .in({ id: req.user._id, date: req.body.date })
      .updateOne({
        $inc: { retweets: -1 },
