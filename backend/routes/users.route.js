...
router.get("/current", auth, (req, res) => {
  User.findById(req.user._id)
    .select("-password -tweets") //find the user in database by id, return without password
...
router.get("/userpage/:username/:limit", (req, res) => {
  User.findOne({ username: req.params.username })
    .select("-password") //find user
    .then((user) => {
      user.tweets = user.tweets || [];
      reverseArray(user.tweets); //display in chronological order
      let end = Math.min(req.params.limit, user.tweets.length);
      let tweetlist = user.tweets.slice(0, end).map((tweet) => {
        return tweet.id;
      }); //get ids of the most recent tweets up to limit

      Tweet.find({ _id: { $in: tweetlist } }) //get data for tweets in tweetlist
        .then((tweets) => {
          let idlist = combineArrays(
            user.following,
            user.followers,
            tweets.map((tweet) => {
              return tweet.original_tweeter;
            })
          ); //get ids of users whose information must be retrieved

          User.find({ _id: { $in: idlist } })
            .select("username _id bio follower_count") //get user information, then return
            .then((other_users) => {
              return res.json({
                user: user,
...
var assembleTimeline = (tweetlists) => {
  let date,
    ...
    timeline = [];
  let top = (i) => {
    return tweetlists.length > 0 && tweetlists[i].length > 0
      ? tweetlists[i][tweetlists[i].length - 1].date
      : -1;
  }; //return most recent tweet from user if list not yet empty
  ...
  User.findById(req.user._id)
    ...
    .then((user) => {
      User.find({ _id: { $in: user.following } })
        .select("username _id bio follower_count tweets")
        .then((following) => {
          let timeline = assembleTimeline(
            following.map((user) => {
              return user.tweets ? user.tweets : [];
            })
          );
          let tweetlist = timeline
            .slice(0, Math.min(req.params.limit, timeline.length))
            .map((tweet) => {
              return tweet.id;
            });
          ...
          Tweet.find({ _id: { $in: tweetlist } }).then((tweets) => {
            let idlist = combineArrays(
              user.followers,
              tweets.map((tweet) => {
                return tweet.original_tweeter;
            ...
            User.find({ _id: { $in: idlist } })
              .select("username _id bio follower_count")
              .then((other_users) => {
                return res.json({
                  user: user,
...
router.get("/finduser/:id", (req, res) => {
  User.findById(req.params.id)
    .select("-password") //find the user in database by id, return without password
    .then((user) => res.json(user))
    .catch((err) => res.status(400).json(err.message));
});
...
router.get("/follow/:id", auth, (req, res) => {
  ...
  query.updateOne(
    { $addToSet: { followers: req.user._id }, $inc: { follower_count: 1 } },
    (err, writeOpResult) => {
      if (err) return res.status(400).json(err);
      response += "Modified: " + writeOpResult.nModified + " ";

      let query2 = User.where({
        _id: req.user._id,
        following: { $nin: req.params.id },
      });
      query2.updateOne(
...
router.get("/unfollow/:id", auth, (req, res) => {
  ...
  User.where({
    ...
    { $pull: { followers: req.user._id }, $inc: { follower_count: -1 } },
    (err, writeOpResult) => {
      if (err) return res.status(400).json(err);
      response += "Modified: " + writeOpResult.nModified + " ";
  ...
  User.findByIdAndDelete(req.params.id)
    ...
    .then((user) => {
      User.updateMany(
        ...
        { $pull: { followers: req.params.id }, $inc: { follower_count: -1 } },
        (err, response) => {
...
router.post("/edit", auth, async (req, res) => {
  ...
  if (error)
    return res
      .status(400)
      .json("Failed validation:" + error.details[0].message);
  ...
  updates = { username: req.body.username, bio: req.body.bio };
    ...
    .then((user) => {
      if (bcrypt.compareSync(req.body.password, user.password)) {
        ...
        res.cookie("x-access-token", token, {
          httpOnly: true,
          maxAge: Date.now().valueOf() + 86400000,
        });
...
router.post("/", async (req, res) => {
  ...
  await User.updateMany(
    { _id: { $in: user.following }, followers: { $nin: saved._id } },
    { $addToSet: { followers: saved._id }, $inc: { follower_count: 1 } },
    (err, res) => {
